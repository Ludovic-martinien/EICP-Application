...
export default function EnseignantMessages() {
  ...
  const fetchContacts = async () => {
    try {
      // Pour un enseignant, on récupère les élèves de ses classes
      ...
      if (uniqueClassIds.size > 0) {
        ...
        studentsData.forEach(s => uniqueStudents.set(s.id, s));
        
        setContacts(Array.from(uniqueStudents.values()).sort((a, b) => a.nom.localeCompare(b.nom)));
      }
    } catch (error) {
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex">
        ...
        <div className="w-1/3 border-r border-slate-200 dark:border-slate-700 flex flex-col">
          <div className="p-4 border-b border-slate-200 dark:border-slate-700">
            <h2 className="font-bold text-slate-900 dark:text-white">Contacts (Élèves)</h2>
          </div>
              ...
              contacts.map(contact => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 truncate capitalize">{contact.role}</p>
                  </div>
                </button>
              ))
            )}
        ...
        <div className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-900/50">
          {selectedContact ? (
            <>
              ...
              <div className="p-4 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 capitalize">{selectedContact.role}</p>
                </div>
              </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-500 dark:text-slate-400">
              ...
              <p className="text-sm">Choisissez un élève pour démarrer une conversation</p>
            </div>
          )}
        </div>
      </div>
